...
void MPU6050Display::CreateGui()
{
    ...
    CtrlIfDisplayBtn = new QPushButton("关闭3D图形绘制");
...
void MPU6050Display::LinkSignalSlot()
{
    ...
    connect(CtrlIfDisplayBtn, &QPushButton::clicked, this, &MPU6050Display::CtrlDrawDatas);
...
void MPU6050Display::CtrlDrawDatas()
{
     if(CtrDrawDatasFlag)
     {
        CtrDrawDatasFlag = false;
        CtrlIfDisplayBtn ->setText("开启3D图形绘制");
     }
     else
     {
         CtrDrawDatasFlag = true;
         CtrlIfDisplayBtn ->setText("关闭3D图形绘制");
     }
}
